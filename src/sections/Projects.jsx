...
const Projects = () => {
  ...
  return (
    <section ref={sectionRef} id="projects" className="py-12 md:py-20 bg-surface relative">
      <div className="max-w-container-max mx-auto px-gutter">
          ...
          <div className="space-y-[40vh] pb-[20vh]">
            {PROJECTS.map((project, index) => (
              <div key={project.id} className={`project-trigger-${index} reveal-project opacity-0 translate-y-10`}>
                <h3 className="text-2xl md:text-4xl font-bold text-on-surface mb-2 md:mb-4">{project.title}</h3>
                <p className="text-secondarytext-[10px] md:text-sm font-bold mb-3 md:mb-4 uppercase tracking-widest">{project.category}</p>
                <p className="text-on-surface-variant text-base md:text-lg mb-6 md:mb-10 leading-relaxed">{project.description}</p>
                ...
                {(project.liveUrl !== "#" || project.githubUrl !== "#") && (
                    <div className="flex gap-4">
                        <a href={project.liveUrl} target='_blank' rel="noopener noreferrer" className="px-5 py-2.5 md:px-6 md:py-3 text-sm md:text-base bg-surface-variant/50 border border-outline/30 rounded-xl flex items-center gap-2 hover:border-secondary/50 transition-all">
                            <span className="material-symbols-outlined text-xl">open_in_new</span> View
                        </a>
                        <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="px-5 py-2.5 md:px-6 md:py-3 text-sm md:text-base bg-surface-variant/50 border border-outline/30 rounded-xl flex items-center gap-2 hover:border-secondary/50 transition-all">
        ...
        {PROJECTS.map((project) => (
            ...
            <div className="p-5 md:p-6 flex flex-col flex-grow">
                <h4 className="text-lg md:text-xl font-bold mb-1 text-on-surface">{project.title}</h4>
                <p className="text-secondary text-[10px] md:text-xs font-bold mb-4 uppercase tracking-widest">
                {project.category}
                </p>
                ...
                {project.tech.map((t, i) => (
                    <div 
                    key={i} 
                    className="w-9 h-9 md:w-10 md:h-10 rounded-full border-1 border-outline bg-surface-dim flex items-center justify-center p-2 shadow-lg"
                    style={{ zIndex: 10 + i }}
                    >
                    <img src={t.icon} alt={t.name} className="w-full h-full object-contain" />
                    </div>
        ...
        {/* Bottom CTA */}
        <div className="mt-12 md:mt-20 pt-8 md:pt-12 border-t border-outline/20 text-center reveal-project opacity-0 translate-y-10">
        <p className="text-on-surface-variant text-sm md:text-base mb-6 md:mb-8 max-w-2xl mx-auto">
        ...
        <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-3 px-6 py-3.5 md:px-8 md:py-4 text-sm md:text-base bg-surface-variant/50 border border-outline/30 rounded-xl font-bold hover:border-secondary hover:text-secondary transition-all group"
        >
            <svg 
