...
const ExperienceItem = ({ item, index }) => {
  return (
    <div className="relative pl-12 pb-12 last:pb-0 reveal-exp opacity-0">
      {/* Timeline Dot */}
      <div 
      ...
      {/* Date */}
      <div className={`text-sm text-${item.color} font-bold mb-2 uppercase tracking-widest font-mono`}>
        {item.period}
      </div>

      {/* Content Card */}
      <div className="group">
        <h3 className="text-2xl font-bold mb-2 text-on-surface group-hover:text-secondary transition-colors">
          {item.role}
        </h3>
        ...
        <ul className="text-on-surface-variant space-y-4 text-lg mb-8">
          {item.description.map((point, i) => (
            <li key={i} className="flex items-start gap-4">
              <span className={`text-${item.color} mt-1.5 material-symbols-outlined text-sm shrink-0`}>
                arrow_forward
              </span>
              <span>{point}</span>
            </li>
...
const Experience = () => {
  ...
  return (
    <section className="py-32 bg-surface-dim/50 overflow-hidden" id="experience" ref={sectionRef}>
      <div className="max-w-container-max mx-auto px-gutter">
        <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-bold text-on-surface reveal-exp">
                Professional Journey
            </h2>
            <div className="w-24 h-1 bg-secondary mx-auto mt-6 reveal-exp"></div>
        </div>
